...
public class PublishController {
    ...
    @GetMapping("/publish/{helloTo}")
    @ResponseBody
    public String publish(@PathVariable("helloTo") String helloTo, @RequestHeader HttpHeaders httpHeaders) {
        Tracer tracer = initTrace();
        try (Scope scope = startServerSpan(tracer, httpHeaders, "publish")) {
            scope.span().getBaggageItem("firstbb");
